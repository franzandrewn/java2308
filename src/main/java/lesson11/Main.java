...
//      stat.executeUpdate("INSERT INTO persons(id, name, age) VALUES (3, 'Yulia', 23)");
  ...
  static void task12() {
    /*
    Создать Maven проект (если нет) и дополнить pom.xml ссылкой на sqlite jdbc
    Найти в google chinook sqlite db (сайт sqlitetutorial.net/), скачать архив с базой данных
    Внутри архива лежит chinook.db, положить его в папку с проектом
    Скопировать код с 9 строки до 11 строки, заменив test.db на chinook.db
    Запустить программу, убедиться, что она выполняется (нет сообщений об SQL EXCEPTION)
     */
    try (Connection connection = DriverManager.getConnection("jdbc:sqlite:chinook.db")) {
      Statement stat = connection.createStatement();
      /*
      На примере строк 14-20 запросить из таблицы artists данные об именах (name) исполнителей
      Вывести имена по одному на строку
       */
